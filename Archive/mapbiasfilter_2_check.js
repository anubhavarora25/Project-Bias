...
let runThis=function (d){
      ...
      function updateChart(selectedGroupObj,passed) {
          console.log(selectedGroupObj,passed)
          ...
          var dataFilter = data.filter(function(d,i){
            let checkGeneration = function() {
              if (selectedGroupObj.hasOwnProperty('generation')) {
                return d.generation === selectedGroupObj.generation || d.generation ==="silent"
              }
            }
            ...
            let checkEducation = function() {
              if (passed.length > 0) {
                  return _.includes(decodeURIComponent(passed), d["education"]);
               } else {
                   data = data; // so that no boxes checked shows all data
               }
            }


            return (
              checkRace() && checkGender() && checkGeneration() && checkEducation()
            )
          })
...
// console.log(countyObj)
      ...
      function renderMap() {
           ...
           //listen to the dropdowns - call update when any of the dropdowns are selected
             ...
             d3.selectAll(".filter-check").on("change", function () {
               svg.selectAll("*").remove()
               ...
               choices=[]
                for (var i = 0; i < checkboxes.length; i++) {
                    ...
                    updateChart(currentState,choices)
              })
